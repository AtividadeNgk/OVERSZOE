...
async def run_bot(token, bot_id):
    ...
    # Configura webhook para detecção de mensagens quando hibernado
    await setup_webhook(token, bot_id)
...
async def setup_webhook(token, bot_id):
    """Configura webhook para o bot"""
    try:
        webhook_url = f"{config.get('url')}/webhook/telegram/{token}"
        ...
        if response.status_code == 200:
            ...
            if result.get("ok"):
                print(f"✅ Webhook configurado para bot {bot_id}: {webhook_url}")
                return True
