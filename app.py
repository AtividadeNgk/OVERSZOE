...
def initialize_all_registered_bots():
    """Inicializa todos os bots registrados e ativos (não hibernados)."""
    ...
    for bot in bots:
        ...
        if manager.is_bot_hibernated(bot_id):
...
def telegram_webhook(bot_token):
    """Webhook unificado para receber updates do Telegram"""
    try:
        # Verifica e acorda o bot se necessário
        check_and_wake_bot(bot_token)
        
        # Processa o update normalmente
        # O bot já estará rodando e processará a mensagem
...
def check_and_wake_bot(bot_token):
    """Verifica se o bot está hibernado e o acorda se necessário"""
    try:
        # Busca o bot pelo token
        bot = None
        ...
        # Verifica se está hibernado
        if manager.is_bot_hibernated(bot_id):
            print(f"Bot {bot_id} está hibernado. Acordando...")
            
            # Remove status de hibernação
            manager.wake_bot(bot_id)
            
            # Inicia o bot
            start_bot(bot_token, bot_id)
            
            print(f"Bot {bot_id} acordado com sucesso!")
            return True
        
        # Verifica se o processo está morto e reinicia
        if bot_id not in processes or not processes[bot_id].is_alive():
            print(f"Bot {bot_id} está com processo morto. Reiniciando...")
        ...
        print(f"Erro ao verificar/acordar bot: {e}")
        return False
    ...
    if update.message and update.message.text:
        ...
        if telegram_bot and telegram_bot.get('result'):
            ...
            if bot_id:
                ...
                # Configura webhook para o novo bot
                webhook_url = f"{config.get('url')}/webhook/telegram/{new_token}"
                requests.post(
...
def bot_webhook_handler(bot_token):
    """Handler global para webhooks de bots - acorda bots hibernados"""
    try:
        ...
        # Verifica e acorda o bot se necessário
        was_hibernated = check_and_wake_bot(token)
        
        if was_hibernated:
            # Se o bot estava hibernado, aguarda um pouco para ele iniciar
            time.sleep(2)
        
        # Pega os dados do update
        update_data = request.get_json()
        
        # Encontra o bot_id pelo token
        bot_id = None
        all_bots = manager.get_all_bots()
        for bot in all_bots:
            if bot[1] == token:
        ...
        if bot_id and bot_id in processes:
            # Se o bot está rodando, deixa ele processar normalmente via polling
            return jsonify({"status": "ok"}), 200
        else:
            # Se não está rodando ainda, tenta acordar novamente
            check_and_wake_bot(token)
            return jsonify({"status": "waking"}), 200
            
    except Exception as e:
        print(f"Erro no webhook handler: {e}")
        return jsonify({"error": str(e)}), 500
...
def get_active_bots():
    if session.get("auth", False):
        ...
        for bot in all_bots:
            ...
            # Verifica se está hibernado
            is_hibernated = manager.is_bot_hibernated(bot_id)
            ...
            bot_info = {
                ...
                'hibernated': is_hibernated
            }
            
            # Se está hibernado, marca como hibernating
            if is_hibernated:
                bot_info['status'] = 'hibernating'
            else:
            ...
            if str(bot_id) in processes:
                try:
                    ...
                    if process:
                        ...
                        process.join(timeout=2)
                    processes.pop(str(bot_id))
                    print(f"Processo {bot_id} hibernado")
                except Exception as e:
                    print(f"Erro ao hibernar processo: {e}")
            
            # Remove dos dados em memória mas NÃO do banco
            if str(bot_id) in bots_data:
                bots_data.pop(str(bot_id))
