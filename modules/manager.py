...
def add_hibernation_column():
    """Adiciona coluna de hibernação se não existir"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    
    # Verifica se a coluna hibernated existe
    cursor.execute("PRAGMA table_info(BOTS)")
    columns = [column[1] for column in cursor.fetchall()]
    
    if 'hibernated' not in columns:
        cursor.execute("ALTER TABLE BOTS ADD COLUMN hibernated INTEGER DEFAULT 0")
        conn.commit()
    
    conn.close()

def hibernate_bot(bot_id):
    """Marca um bot como hibernado"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    
    # Garante que a coluna existe
    add_hibernation_column()
    
    cursor.execute("UPDATE BOTS SET hibernated = 1 WHERE id = ?", (bot_id,))
    conn.commit()
    conn.close()
    return True

def wake_bot(bot_id):
    """Remove o status de hibernação de um bot"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    
    cursor.execute("UPDATE BOTS SET hibernated = 0 WHERE id = ?", (bot_id,))
    conn.commit()
    conn.close()
    return True

def is_bot_hibernated(bot_id):
    """Verifica se um bot está hibernado"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    ...
    cursor.execute("SELECT hibernated FROM BOTS WHERE id = ?", (bot_id,))
    result = cursor.fetchone()
    conn.close()
...
def get_all_hibernated_bots():
    """Retorna todos os bots hibernados"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    ...
    cursor.execute("SELECT * FROM BOTS WHERE hibernated = 1")
